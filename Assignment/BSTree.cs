...
{
	//Creates a binary tree object with which to store the library members and contains methods to keep
	//the tree sorted
	...
	public class BSTree
	{
		...
		public string ReturnPhone(Member member)
		{
			return ReturnPhone(member, root);
		}

		private string ReturnPhone(Member member, BTreeNode r)
		{
			if (r != null)
			{
				if (member.CompareTo(r.Member) == 0)
					return r.Member.ContactNumber;
				else
					if (member.CompareTo(r.Member) < 0)
					return ReturnPhone(member, r.LChild);
				else
					return ReturnPhone(member, r.RChild);
			}
			else
				return "Not found!";
		}
		...
		private void PreOrderTraverse(BTreeNode root)
		{
			if (root != null)
			{
				Program.memberList.Add(root.Member);
		...
		private void PostOrderTraverse(BTreeNode root)
		{
			if (root != null)
			{
				...
				Console.Write(root.Member.ToString());
			}
		}
